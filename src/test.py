...
from mv_train_two import draw_gt_pred, get_concat_img
import os
import csv
...
def get_data(input_path):
    ...
    with open(input_path,'r') as f:
        ...
        for line in tqdm(f):
            ...
            (filename,x1,y1,x2,y2,class_idx) = line_split
            class_name = messytable_class_list[int(class_idx)-1]
...
parser.add_argument('--base_path', type=str, default = '/data3/sap/frcnn_keras')
...
messytable_class_list = list(messytable_class_mapping.keys())
C.class_mapping = {messytable_class_list[int(key)-1] : value for key, value in list(C.class_mapping.items())[:-1]}
C.class_mapping['bg'] = len(C.class_mapping)
C.class_mapping = {key : C.class_mapping[key] for key in messytable_class_list}
...
if args.load_pickle :
    f = open(test_path.replace('txt', 'pickle'), 'rb')
...
T = {cls : [] for cls in list(C.class_mapping.keys())[:-1]}
P = {cls : [] for cls in list(C.class_mapping.keys())[:-1]}
mAPs = []
elapsed_time = []
img_list = []
progbar = generic_utils.Progbar(len(test_imgs)) if args.progbar else None
print('\n', C.model_path, 'calculating...')
for idx, img_data in enumerate(test_imgs):
    ...
    for key in bboxes:
        ...
        for jk in range(new_boxes.shape[0]):
            ...
            if args.log_output :
                ...
                #output_log.write([filepath, real_x1, real_y1, real_x2, real_y2, round(prob, 2), key])
                output_log.write([filepath, real_x1, real_y1, real_x2, real_y2, prob, key])
    ...
    if args.save_img :
        if idx % 3 == 0 and idx:
            result_img = get_concat_img(img_list)
        ...
        #img = draw_gt_pred(img, all_dets, img_data['bboxes'], ratio, save_path)
        img = draw_gt_pred(img, all_dets, {}, ratio)
        img_list.append(img)
...
if args.val :
    ...
    for key in list(C.class_mapping.keys())[:-1]:
        ...
        all_aps.append(ap)
    print('mAP = {}'.format(np.mean(np.array(all_aps))))
    print('elapsed time = {}'.format(np.sum(np.array(elapsed_time))))
