                ...
                for jy in range(output_height):
                    ...
                    for bbox_num in range(num_bboxes):
                        ...
                        if curr_iou > best_iou_for_bbox[bbox_num] or curr_iou > C.rpn_max_overlap:
                            #if jy == 9 and ix == 40 and anchor_ratio_idx == 0 :
                            #    print('check')
...
#model_rpn = multi_gpu_model(model_rpn, gpus=3)
model_rpn.compile(optimizer=optimizer, loss=[rpn_loss_cls(num_anchors), rpn_loss_regr(num_anchors)])
#model_classifier = multi_gpu_model(model_classifier, gpus=3)
...
#                 print('Average number of overlapping bounding boxes from RPN = {} for {} previous iterations'.format(mean_overlapping_bboxes, epoch_length))
            ...
            '''
            path = '/home/sap/frcnn_keras_original/sv_rpn.pickle'
            f = open(path, 'wb')
            pickle.dump(P_rpn,f)

            path = '/home/sap/frcnn_keras_original/sv_gt.pickle'
            f = open(path, 'wb')
            pickle.dump(Y,f)
            '''
            ...
            if iter_num == epoch_length:
                ...
                #        print('Total loss decreased from {} to {}, saving weights'.format(best_loss,curr_loss))
                    best_loss = curr_loss
                model_all.save_weights(C.model_path %(epoch_num))
